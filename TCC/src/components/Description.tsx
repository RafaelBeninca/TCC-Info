...
const Description = () => {
  ...
  return (
    <div className="w-full h-full">
      ...
      <div className="flex flex-row h-1/2 w-full ml-7">
        <textarea
          className={editable ? "flex flex-row bg-slate-200 rounded-md resize-none focus:ring-primary-default" : "bg-transparent rounded-md resize-none"}
          style={{ width: "80%" }}
          disabled={!editable}
        ...
        <div className="flex flex-col gap-0" >
          {editable == true && (
          <div>
            <button className="flex flex-row w-10 h-10 hover:scale-110 transition-all duration-300" onClick={saveEdit}>
      ...
      {(user?.uid !== userId || editable == false) && (
        <div className="flex flex-row w-full h-full">
          <p style={{ width: "70%" }} className="h-1/2 w-2/3 ml-7 whitespace-normal line-clamp-6 break-words text-ellipsis"> {description} </p>
          {user?.uid == userId ?
